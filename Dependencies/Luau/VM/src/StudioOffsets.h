...
static std::uintptr_t luau_execute = 0xDEADBEEF;

// Get to lua_pushvalue and get inside of pseudo2addr, you will find the data xref.
static std::uintptr_t _luaO_nilobject = 0xDEADBEEF;
...
// 41 8D 41 ? 48 98 48 83 C0 ? 48 C1 E0 ? 49 03 C0 C3
static std::uintptr_t pseudo2addr = 0xDEADBEEF;
...
static std::uintptr_t luaE_newthread = 0xDEADBEEF;
...
static std::uintptr_t luaD_throw = 0xDEADBEEF;
...
static std::uintptr_t lua_newthread = 0xDEADBEEF;

// search for "defer"
static std::uintptr_t rTask_defer = 0xDEADBEEF;
...
static std::uintptr_t rFreeBlock = 0xDEADBEEF;
...
// Signature: 74 4A 48 8B CD E8 ? ? ? ? 48 8B F0 B9 ? ? ? ? E8 ? ? ? ? 48 89 44 24 ? 48 85 C0 74 10 48 8B D6 48 8B C8 E8 ? ? ? ? 48 8B F0 EB 02 33 F6
// 48 8B CD E8 ? ? ? ? 48 89 46 ? 48 8B D6 48 8B CF E8 ? ? ? ?
static std::uintptr_t rFromLuaState = 0xDEADBEEF;
...
static std::uintptr_t luaD_rawrununprotected = 0xDEADBEEF;
...
// luaC_checkgc, which calls luaC_step! Signature: 48 8B 59 ? B8 ? ? ? ? 0F B6 F2 0F 29 74 24 ? 4C 8B F1 44 8B 43 ? 44 0F AF 43 ? 48 8B 6B ? 48 2B 6B
// ? 41 F7 E8 8B FA C1 FF ? 8B C7 C1 E8 ? 03 F8 48 8B 83 ? ? ? ? 48 85 C0 74 04 33 D2 FF D0 0F B6 43 ? 84 C0 75 32
static std::uintptr_t luaC_Step = 0xDEADBEEF;

// search for "ProximityPrompt_Triggered".
static std::uintptr_t fireproximityprompt = 0xDEADBEEF;

// search for "InvalidInstance". Caller with two arguments (Modifies lua stack)
static std::uintptr_t pushinstance = 0xDEADBEEF;

// Search for "TaskSchedulerTargetFps". Use the .data segment reference it refers towards. It is a DWORD (or int32_t)
static std::uintptr_t FFlag_TaskSchedulerTargetFps = 0xDEADBEEF;
} // namespace RBX::Studio::Offsets
...
{
static FunctionTypes::luau_execute luau_execute;
static FunctionTypes::luaE_newthread luaE_newthread;
static FunctionTypes::luaD_throw luaD_throw;
static FunctionTypes::luaD_rawrununprotected luaD_rawrununprotected;
static FunctionTypes::luaC_Step luaC_Step;
} // namespace RBX::Studio::Functions
